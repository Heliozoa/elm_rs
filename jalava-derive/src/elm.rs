...
fn intermediate_to_token_stream(
    ...
    let type_definition = match kind {
        ...
        TypeKind::Struct(fs) => struct_type(&elm_type, fs),
...
fn struct_type(elm_type: &str, fs: Vec<(Ident, Type)>) -> TokenStream2 {
...
fn enum_type(elm_type: &str, vs: Vec<(Ident, EnumVariant)>) -> TokenStream2 {
