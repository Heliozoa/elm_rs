...
impl_builtin_container!(std::collections::BTreeMap<String,T>, "Dict", "Json.Decode.dict", "Json.Encode.dict");
...
// todo duration
impl_builtin_container!(std::collections::HashMap<String,T>, "Dict", "Json.Decode.dict", "Json.Encode.dict");
impl_builtin_container!(
...
#[cfg(feature = "chrono")]
impl<T: chrono::TimeZone> Elm for chrono::Date<T>
where
    T: Elm,
{
    fn elm_type() -> String {
...
macro_rules! export {
    ($target: expr, $($type: path),*) => {
        {
            fn func(target: &mut impl std::io::Write) -> Result<(), std::io::Error> {
                use std::io::Write;
                use jalava::Elm;
                writeln!(target,
"-- GENERATED BY JALAVA
...
")?;
                $(
                    writeln!(target, "{}\n", <$type>::elm_definition().unwrap())?;
                    writeln!(target, "{}\n", <$type>::encoder_definition().unwrap())?;
                    writeln!(target, "{}\n", <$type>::decoder_definition().unwrap())?;
                )*
                Ok(())
            }
            func($target)
